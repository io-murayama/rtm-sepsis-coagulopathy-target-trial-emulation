...
with 
  infect_pts as (
    ...
    from `medicu-beta.snapshots_one_icu.icu_diagnoses_20260706`
    where 
  ...
  day1_sofa as (
    ...
    from `medicu-beta.snapshots_one_icu_derived.sofa_hourly_20260706`
  ...
  vaso as (
    ...
    from `medicu-beta.snapshots_one_icu_derived.infusion_injection_active_ingredient_rate_20260706`
  ...
  mv as (
    ...
    from medicu-beta.snapshots_one_icu.mechanical_ventilations_20260706
    group by icu_stay_id
  ),
    ...
    with
      platelet_measurements as (
        select
          ...
          value
        from medicu-beta.snapshots_one_icu.laboratory_tests_blood_20260706
        inner join medicu-beta.snapshots_one_icu.icu_stays_20260706 using (icu_stay_id)
  ...
  exclusion_criteria as (
      with
          ...
          antithrombin_before as (
              ...
              left join medicu-beta.snapshots_one_icu_derived.infusion_injection_active_ingredient_rate_20260706 t using (icu_stay_id)
          ...
          younger as (
              ...
              where
                icu_stay_id not in (
                  select icu_stay_id
                  from medicu-beta.snapshots_one_icu_derived.extended_icu_stays_20260706
                  where age >= 18
                )
          ),
          ...
          liver_disease as (
              ...
              where
                icu_stay_id in (
                  select icu_stay_id
                  from `medicu-beta.snapshots_one_icu_derived.charlson_20260706`
          ...
          no_recorded_vital_measurement as (
              ...
              where
                  icu_stay_id not in (
                      select distinct icu_stay_id
                      from
                          `medicu-beta.snapshots_one_icu_derived.aggregated_vital_measurements_20260706`
                      where
...
select distinct icu_stay_id from exclusion_criteria
